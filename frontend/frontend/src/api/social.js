...
export const studyGroupAPI = {
  ...
  searchGroups: (keyword, page = 0, size = 10) => 
    api.get(`/study-groups/search/name?keyword=${keyword}&page=${page}&size=${size}`),
  
  // 根据学科获取学习小组
  getGroupsBySubject: (subject, page = 0, size = 10) => 
    api.get(`/study-groups/search/subject?subject=${subject}&page=${page}&size=${size}`),
...
export const studyShareAPI = {
  ...
  getSharesByTags: (tags, page = 0, size = 10) => 
    api.get(`/study-shares/search/tag?tag=${tags}&page=${page}&size=${size}`),
  ...
  // 根据类型和学科同时筛选学习分享
  getSharesByTypeAndSubject: (type, subject, page = 0, size = 10) => 
    api.get(`/study-shares/filter?type=${type}&subject=${subject}&page=${page}&size=${size}`),
  
  // 根据标签搜索学习分享
  getSharesByTags: (tag, page = 0, size = 10) => 
    api.get(`/study-shares/search/tag?tag=${tag}&page=${page}&size=${size}`)
}
...
export const qaAPI = {
  ...
  getQuestions: (page = 0, size = 10, viewMode = 'latest', userId = null) => {
    ...
    switch (viewMode) {
      ...
      case 'recommended':
        return api.get(`/qa/questions/recommended/likes?page=${page}&size=${size}`);
      case 'closed':
        return userId ? api.get(`/qa/questions/closed?page=${page}&size=${size}&userId=${userId}`) : api.get(`/qa/questions/closed?page=${page}&size=${size}&userId=0`);
      case 'latest':
      default:
  ...
  // 更新问题状态
  updateQuestionStatus: (questionId, status, userId) => 
    api.put(`/qa/questions/${questionId}/status?status=${status}&userId=${userId}`),
  
  // 关闭问题
  closeQuestion: (questionId, userId) => 
    api.put(`/qa/questions/${questionId}/close?userId=${userId}`),
  
  // 获取已关闭的问题
  getClosedQuestions: (userId, page = 0, size = 10) => 
    api.get(`/qa/questions/closed?page=${page}&size=${size}&userId=${userId}`),
  
  // 重新开启问题
  reopenQuestion: (questionId, userId) => 
    api.put(`/qa/questions/${questionId}/reopen?userId=${userId}`)
}
