  ...
  <div class="social-learning">
    ...
    <div class="feature-nav">
      <el-tabs v-model="activeTab" @tab-change="handleTabChange">
        ...
        <el-tab-pane label="学习排行榜" name="leaderboard">
          <template #label>
            <span class="tab-label">
    ...
    <div v-if="activeTab === 'overview'" class="overview-section">
      <el-row :gutter="20">
        ...
        <el-col :span="6">
          <el-card shadow="hover" class="feature-card" @click="activeTab = 'leaderboard'">
            <div class="feature-content">
              ...
              <div class="feature-info">
                <h3>学习排行榜</h3>
                <p>查看学习进度排名，与好友比较成就</p>
                <div class="feature-stats">
                  <span>排名 #{{ leaderboardStats.rank }}</span>
                </div>
              </div>
            </div>
          </el-card>
    ...
    <!-- 快速统计 -->
    <div class="quick-stats" v-if="activeTab !== 'overview'">
      <el-row :gutter="15">
        <el-col :span="6">
          <div class="stat-item">
            <div class="stat-icon">
            ...
            <div class="stat-info">
              <div class="stat-number">{{ groupStats.joined }}</div>
        ...
        <el-col :span="6">
          <div class="stat-item">
            ...
            <div class="stat-info">
              <div class="stat-number">{{ shareStats.count }}</div>
              <div class="stat-label">学习分享</div>
            </div>
          </div>
        </el-col>
        <el-col :span="6">
          <div class="stat-item">
            ...
            <div class="stat-info">
              <div class="stat-number">{{ qaStats.questions }}</div>
              <div class="stat-label">提出问题</div>
            </div>
          </div>
        </el-col>
        <el-col :span="6">
          <div class="stat-item">
            ...
            <div class="stat-info">
              <div class="stat-number">#{{ leaderboardStats.rank }}</div>
    ...
    <div class="sidebar-actions">
      <el-affix :offset="80">
        <div class="action-buttons">
          ...
          <el-tooltip content="查看排行" placement="left">
            <el-button 
              type="info" 
              circle 
              size="large"
              @click="quickAction('viewLeaderboard')"
            >
              <el-icon><Trophy /></el-icon>
            </el-button>
          </el-tooltip>
...
import { 
  UserFilled, Share, QuestionFilled, Trophy, Plus, Edit, Refresh, Bell 
...
export default {
  ...
  setup() {
    ...
    const leaderboardStats = reactive({
      rank: 0,
      points: 0
    })
    ...
    const loadStats = async () => {
      try {
        // 加载用户学习统计
        const statsResponse = await axios.get(`http://localhost:8080/api/leaderboard/stats/${currentUserId.value}`)
        if (statsResponse.data.success) {
          ...
          qaStats.answers = stats.questionsAnswered || 0
        }
        
        // 加载排名信息
        const rankResponse = await axios.get('http://localhost:8080/api/leaderboard/rank/points', {
          params: { userId: currentUserId.value }
        })
        if (rankResponse.data.success) {
          leaderboardStats.rank = rankResponse.data.data || 999
        }
      } catch (error) {
