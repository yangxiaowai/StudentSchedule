<template>
  <div class="study-shares">
    ...
    >
      <div v-if="selectedShare" class="share-detail dialog-content">
  ...
  setup() {
    ...
    const searchShares = async () => {
      ...
      try {
        ...
        if (response.data && response.data.success) {
          const shareList = response.data.data
          shares.value = shareList.map(share => ({
            ...share,
            isLiked: false
          }))
          // 为每个分享获取用户名
    ...
    const filterShares = async () => {
      try {
        ...
        // 如果有筛选条件，使用筛选API
        if (selectedType.value && selectedSubject.value) {
          // 同时按类型和学科筛选
          response = await studyShareAPI.getSharesByTypeAndSubject(selectedType.value, selectedSubject.value, page, size)
