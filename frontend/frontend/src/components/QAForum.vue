        ...
        <div class="question-header">
          ...
          <div class="question-actions">
            ...
            <el-button 
              ...
              size="small" 
              type="success"
              @click="reopenQuestion(question.id)"
            >
    ...
    >
          ...
          <div class="answers-header">
            ...
            <div class="answers-header-right">
              ...
              <el-button 
                ...
                :class="{ 'disabled-button': selectedQuestion.status === 'CLOSED' }"
                @click="selectedQuestion.status !== 'CLOSED' ? showAnswerDialog = true : null"
              >
                <el-icon><Plus /></el-icon>
                回答问题
              </el-button>
              ...
              <div class="answer-header">
                ...
                <div class="answer-actions">
                  <el-button 
                    ...
                    :disabled="selectedQuestion.status === 'CLOSED'"
                    :class="{ 'disabled-button': selectedQuestion.status === 'CLOSED' }"
                    @click="selectedQuestion.status !== 'CLOSED' ? toggleAnswerLike(answer) : null"
                  >
                    <el-icon><Star /></el-icon>
                    {{ answer.likeCount }}
                  </el-button>
                  <!-- 最佳答案现在根据点赞数自动确定，无需手动设置 -->
                </div>
              </div>
      ...
      <template #footer>
        <span class="dialog-footer">
          ...
          <el-button 
            v-if="selectedQuestion && selectedQuestion.userId === currentUserId && selectedQuestion.status !== 'RESOLVED'"
            type="success" 
            :disabled="selectedQuestion.status === 'CLOSED'"
            :class="{ 'disabled-button': selectedQuestion.status === 'CLOSED' }"
            @click="selectedQuestion.status !== 'CLOSED' ? markAsResolved(selectedQuestion) : null"
          >
            标记为已解决
          </el-button>
          <el-button 
            ...
            :class="{ 'disabled-button': selectedQuestion && selectedQuestion.status === 'CLOSED' }"
            @click="selectedQuestion && selectedQuestion.status !== 'CLOSED' ? toggleLike(selectedQuestion) : null"
          >
...
import { Plus, Search, Star, View, ChatDotRound, Trophy, Close, Refresh } from '@element-plus/icons-vue'
...
export default {
  ...
  setup() {
    ...
    const newAnswer = reactive({
      ...
      imageUrl: '',
      userId: parseInt(localStorage.getItem('userId')) || 51,
      answererId: parseInt(localStorage.getItem('userId')) || 51,
      questionId: null
    })
    ...
    const filterQuestions = async () => {
      try {
        // 如果没有任何筛选条件，则加载所有问题
        if (!selectedStatus.value && !selectedSubject.value && !selectedDifficulty.value) {
          loadQuestions()
          return
        }
    ...
    const createAnswer = async () => {
      try {
        ...
        if (response.data.success) {
          ...
          ElMessage.error('回答失败: ' + (response.data.message || '未知错误'))
        }
      } catch (error) {
        console.error('请求失败:', error)
        console.error('错误详情:', error.response?.data)
        ElMessage.error('回答失败: ' + (error.response?.data?.message || error.message || '网络错误'))
      }
    }
    ...
    const markAsResolved = async (question) => {
      try {
        const response = await qaAPI.updateQuestionStatus(question.id, 'RESOLVED', currentUserId.value)
    ...
    const closeQuestion = async (questionId) => {
      try {
        const response = await qaAPI.closeQuestion(questionId, currentUserId.value)
        if (response.data.success) {
          ElMessage.success('问题已关闭')
          loadQuestions() // 重新加载问题列表
        } else {
          ElMessage.error(response.data.message || '关闭问题失败')
        }
      } catch (error) {
    ...
    const reopenQuestion = async (questionId) => {
      try {
        const response = await qaAPI.reopenQuestion(questionId, currentUserId.value)
        if (response.data.success) {
          ElMessage.success('问题已重新开启')
          loadQuestions() // 重新加载问题列表
        } else {
          ElMessage.error(response.data.message || '重新开启问题失败')
        }
      } catch (error) {
        console.error('重新开启问题失败:', error)
        ElMessage.error('重新开启问题失败')
      }
    }
...
.question-detail-dialog .el-dialog,
    ...
    .question-detail-dialog .el-dialog__header,
    .new-question-dialog .el-dialog__header {
      flex-shrink: 0;
      border-bottom: 1px solid #ebeef5;
    }
    
    .question-detail-dialog .el-dialog__footer,
    .new-question-dialog .el-dialog__footer {
      flex-shrink: 0;
    ...
    .scrollable-dialog .el-dialog__header {
      flex-shrink: 0;
      border-bottom: 1px solid #ebeef5;
    }

    .scrollable-dialog .el-dialog__footer {
      flex-shrink: 0;
    ...
    .question-detail-dialog .el-dialog__body::-webkit-scrollbar,
    .new-question-dialog .el-dialog__body::-webkit-scrollbar {
      width: 8px;
    }

    .scrollable-dialog .el-dialog__body::-webkit-scrollbar-track,
    .question-detail-dialog .el-dialog__body::-webkit-scrollbar-track,
    .new-question-dialog .el-dialog__body::-webkit-scrollbar-track {
      background: #f1f1f1;
      border-radius: 4px;
    }

    .scrollable-dialog .el-dialog__body::-webkit-scrollbar-thumb,
    .question-detail-dialog .el-dialog__body::-webkit-scrollbar-thumb,
    .new-question-dialog .el-dialog__body::-webkit-scrollbar-thumb {
      background: #c1c1c1;
      border-radius: 4px;
    ...
    .question-detail-dialog .el-dialog__body::-webkit-scrollbar-thumb:hover,
    .new-question-dialog .el-dialog__body::-webkit-scrollbar-thumb:hover {
      background: #a8a8a8;
    }
