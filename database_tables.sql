...
CREATE TABLE IF NOT EXISTS `task` (
    ...
    `completed` BOOLEAN DEFAULT FALSE COMMENT '任务是否完成',
    `content_type` VARCHAR(50) COMMENT '内容类型',
    `remark` TEXT COMMENT '备注',
    `file_url` VARCHAR(500) COMMENT '上传文件的访问路径或URL',
    `file_name` VARCHAR(255) COMMENT '上传文件的原始名称',
...
CREATE TABLE IF NOT EXISTS `user_stats` (
    ...
    `study_hours_total` DECIMAL(10,2) DEFAULT 0 COMMENT '总学习时长(小时)',
    `study_hours_weekly` DECIMAL(10,2) DEFAULT 0 COMMENT '本周学习时长',
    `study_hours_monthly` DECIMAL(10,2) DEFAULT 0 COMMENT '本月学习时长',
...
-- 确保qa_question和qa_answer表存在且结构正确
-- 先删除有外键依赖的表
...
-- 插入问答测试数据
INSERT INTO `qa_question` (`title`, `content`, `subject`, `tags`, `asker_id`, `views_count`, `answers_count`) VALUES
...
-- 插入问答回答测试数据
INSERT INTO `qa_answer` (`question_id`, `content`, `answerer_id`, `is_best_answer`) VALUES
((SELECT `id` FROM `qa_question` WHERE `title` = '如何提高数学成绩？'), '建议多做练习题，理解概念比死记硬背更重要。可以尝试费曼学习法。', (SELECT `id` FROM `user` WHERE `username` = 'admin'), TRUE),
((SELECT `id` FROM `qa_question` WHERE `title` = '如何提高数学成绩？'), '我觉得找到适合自己的学习方法很重要，可以试试不同的方法。', (SELECT `id` FROM `user` WHERE `username` = 'student2'), FALSE),
((SELECT `id` FROM `qa_question` WHERE `title` = '英语听力怎么练习？'), '推荐多听英语播客和看英文电影，从简单的开始。', (SELECT `id` FROM `user` WHERE `username` = 'admin'), TRUE),
((SELECT `id` FROM `qa_question` WHERE `title` = '编程入门应该学什么语言？'), 'Python是很好的入门语言，语法简单易懂。', (SELECT `id` FROM `user` WHERE `username` = 'admin'), TRUE),
((SELECT `id` FROM `qa_question` WHERE `title` = '编程入门应该学什么语言？'), 'JavaScript也不错，可以做网页开发。', (SELECT `id` FROM `user` WHERE `username` = 'student2'), FALSE);

-- 更新问题的回答数和最佳答案
UPDATE `qa_question` SET 
    `answers_count` = (SELECT COUNT(*) FROM `qa_answer` WHERE `qa_answer`.`question_id` = `qa_question`.`id`),
