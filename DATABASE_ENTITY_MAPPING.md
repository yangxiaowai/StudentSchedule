...
| `user_learning_stats` | `UserLearningStats.java` | 🆕 新创建 | 用户学习统计表 |
...
#### 2. user_stats vs user_learning_stats
- **user_stats**: 包含基础统计信息
- **user_learning_stats**: 包含更详细的学习统计信息
- **建议**: 可以合并或根据不同用途分别使用
...
id -> id (bigint, auto_increment)
userId -> user_id (bigint)
totalStudyHours -> total_study_hours (decimal(8,2))
weeklyStudyHours -> weekly_study_hours (decimal(6,2))
monthlyStudyHours -> monthly_study_hours (decimal(7,2))
totalPoints -> total_points (int)
currentLevel -> current_level (int)
currentStreak -> current_streak (int)
longestStreak -> longest_streak (int)
totalTasksCompleted -> total_tasks_completed (int)
bestAnswersCount -> best_answers_count (int)
sharesCount -> shares_count (int)
likesReceived -> likes_received (int)
