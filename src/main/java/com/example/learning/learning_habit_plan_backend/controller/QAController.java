...
public class QAController {
    ...
    @Operation(summary = "获取按点赞数排序的推荐问题")
    @GetMapping("/questions/recommended/likes")
    public ResponseEntity<Map<String, Object>> getRecommendedQuestionsByLikes(
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "10") int size) {
        ...
        try {
            Pageable pageable = PageRequest.of(page, size);
            Page<QAQuestion> questions = qaService.getRecommendedQuestionsByLikes(pageable);
    ...
    @Operation(summary = "获取用户推荐问题")
    @GetMapping("/questions/recommendations/{userId}")
    public ResponseEntity<Map<String, Object>> getUserRecommendedQuestions(
            @PathVariable Long userId,
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "10") int size) {
        ...
        try {
            ...
            response.put("success", false);
            response.put("message", "获取推荐问题失败: " + e.getMessage());
    ...
    public ResponseEntity<Map<String, Object>> updateQuestionStatus(
            ...
            @RequestParam QAQuestion.QuestionStatus status,
            @RequestParam Long userId) {
        
        Map<String, Object> response = new HashMap<>();
        try {
            boolean success = qaService.updateQuestionStatus(questionId, status, userId);
            if (success) {
                response.put("success", true);
                response.put("message", "问题状态更新成功");
            } else {
                response.put("success", false);
                response.put("message", "更新失败，只有提问者可以更新问题状态");
            }
            return ResponseEntity.ok(response);
        } catch (Exception e) {
            response.put("success", false);
    ...
    public ResponseEntity<Map<String, Object>> closeQuestion(
            @PathVariable Long questionId,
            @RequestParam Long userId) {
        
        Map<String, Object> response = new HashMap<>();
        try {
            boolean success = qaService.closeQuestion(questionId, userId);
            if (success) {
                response.put("success", true);
                response.put("message", "问题已关闭");
                return ResponseEntity.ok(response);
            } else {
                response.put("success", false);
                response.put("message", "关闭失败，只有问题发布者可以关闭问题");
                return ResponseEntity.badRequest().body(response);
            }
        } catch (Exception e) {
            response.put("success", false);
    ...
    @Operation(summary = "获取已关闭的问题")
    @GetMapping("/questions/closed")
    public ResponseEntity<Map<String, Object>> getClosedQuestions(
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "10") int size,
            @RequestParam Long userId) {
        ...
        try {
            Pageable pageable = PageRequest.of(page, size);
            Page<QAQuestion> questions = qaService.getClosedQuestionsByUser(userId, pageable);
    ...
    @Operation(summary = "重新开启问题")
    @PutMapping("/questions/{questionId}/reopen")
    public ResponseEntity<Map<String, Object>> reopenQuestion(
            @PathVariable Long questionId,
            @RequestParam Long userId) {
        
        Map<String, Object> response = new HashMap<>();
        try {
            boolean success = qaService.reopenQuestion(questionId, userId);
            if (success) {
                response.put("success", true);
                response.put("message", "问题已重新开启");
                return ResponseEntity.ok(response);
            } else {
                response.put("success", false);
                response.put("message", "重新开启失败，只有问题发布者可以重新开启问题");
                return ResponseEntity.badRequest().body(response);
            }
        } catch (Exception e) {
            response.put("success", false);
            response.put("message", "重新开启失败: " + e.getMessage());
            return ResponseEntity.badRequest().body(response);
        }
    }
