...
public class StudyShareController {
    ...
    @Operation(summary = "根据类型和学科筛选分享")
    @GetMapping("/filter")
    public ResponseEntity<Map<String, Object>> getSharesByTypeAndSubject(
        ...
        try {
            Pageable pageable = PageRequest.of(page, size);
            Page<StudyShare> shares;
            
            if (type != null && subject != null) {
                // 同时根据类型和学科筛选
                StudyShare.ShareType shareType = StudyShare.ShareType.valueOf(type.toUpperCase());
                shares = studyShareService.getSharesByTypeAndSubject(shareType, subject, pageable);
            } else if (type != null) {
                // 只根据类型筛选
                StudyShare.ShareType shareType = StudyShare.ShareType.valueOf(type.toUpperCase());
                shares = studyShareService.getSharesByType(shareType, pageable);
            } else if (subject != null) {
                // 只根据学科筛选
                shares = studyShareService.getSharesBySubject(subject, pageable);
            } else {
                // 没有筛选条件，返回所有公开分享
                shares = studyShareService.getPublicShares(pageable);
            }
