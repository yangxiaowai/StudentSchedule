...
public interface StudyShareRepository extends JpaRepository<StudyShare, Long> {
    ...
    // 根据分享类型和学科同时查找
    Page<StudyShare> findByShareTypeAndSubjectAndIsPublicTrueOrderByCreatedAtDesc(StudyShare.ShareType shareType, String subject, Pageable pageable);
    
    // 查找小组内的分享
    Page<StudyShare> findByGroupIdOrderByCreatedAtDesc(Long groupId, Pageable pageable);
    
    // 根据标题模糊查询
    @Query("SELECT ss FROM StudyShare ss WHERE ss.title LIKE CONCAT('%', :keyword, '%') AND ss.isPublic = true ORDER BY ss.createdAt DESC")
    ...
    // 根据标签查找分享
    @Query("SELECT ss FROM StudyShare ss WHERE ss.tags LIKE CONCAT('%', :tag, '%') AND ss.isPublic = true ORDER BY ss.createdAt DESC")
