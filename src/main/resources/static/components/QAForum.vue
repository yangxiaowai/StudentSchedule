  ...
  setup() {
    ...
    const loadQuestions = async () => {
      try {
        ...
        } else if (viewMode.value === 'unresolved') {
          url += '/unresolved'
        }
    ...
    const loadAnswers = async (questionId) => {
      try {
        const response = await axios.get(`/api/qa/answers/question/${questionId}`)
        if (response.data.success) {
          answers.value = response.data.data.map(answer => ({
            ...answer,
            isLiked: false // 这里应该从后端获取用户是否已点赞
          }))
        }
    ...
    const searchQuestions = async () => {
      ...
      try {
        const response = await axios.get('/api/qa/questions/search', {
          params: {
    ...
    const searchByTag = async (tag) => {
      try {
        const response = await axios.get('/api/qa/questions/search/tag', {
          params: {
            tag: tag,
            page: currentPage.value - 1,
            size: pageSize.value
    ...
    const createQuestion = async () => {
      try {
        newQuestion.userId = currentUserId.value
        const response = await axios.post('/api/qa/questions', newQuestion)
        if (response.data.success) {
    ...
    const createAnswer = async () => {
      try {
        ...
        newAnswer.userId = currentUserId.value
        const response = await axios.post('/api/qa/answers', newAnswer)
        if (response.data.success) {
    ...
    const toggleLike = async (question) => {
      try {
        const url = `/api/qa/questions/${question.id}/like`
        const method = question.isLiked ? 'delete' : 'post'
    ...
    const toggleAnswerLike = async (answer) => {
      try {
        const url = `/api/qa/answers/${answer.id}/like`
        const method = answer.isLiked ? 'delete' : 'post'
    ...
    const setBestAnswer = async (answer) => {
      try {
        const response = await axios.post(`/api/qa/answers/${answer.id}/best`, null, {
          params: { userId: currentUserId.value }
        })
    ...
    const viewQuestionDetail = async (question) => {
      ...
      // 增加浏览量
      try {
        await axios.post(`/api/qa/questions/${question.id}/view`)
